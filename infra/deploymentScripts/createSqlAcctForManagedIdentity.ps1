...
try {
    New-AzSqlServerFirewallRule -ResourceGroupName $ResourceGroupName -ServerName $ServerName -FirewallRuleName "Allow Deployment Script IP" -StartIpAddress $localIP -EndIpAddress $localIP
    ...
    Invoke-Sqlcmd -ServerInstance $ServerUri -database $DatabaseName -Username $SqlAdminLogin -Password $SqlAdminPwd -Query $CreateUserSQL
    ...
    # Connect as SQL Admin acct and execute SQL cmd
    Invoke-Sqlcmd -ServerInstance $ServerUri -database $DatabaseName -Username $SqlAdminLogin -Password $SqlAdminPwd -Query $GrantDbOwner
    ...
    Remove-AzSqlServerFirewallRule -ResourceGroupName $ResourceGroupName -ServerName $ServerName -FirewallRuleName "Allow Deployment Script IP"
}
