...
class ConsolidatedFrontendTests(unittest.TestCase):
    ...
    def test_production_azd_configuration_deploys_only_the_api_service(self) -> None:
        ...
        self.assertIn("Deploy Application Without Custom Domain", pipeline)
        self.assertIn("Configure Container App Domain Validation", pipeline)
        self.assertIn("Bind Container App Custom Domain", pipeline)
        self.assertIn("properties.customDomainVerificationId", pipeline)
        self.assertIn("asuid.fakesurveygenerator", pipeline)
        self.assertIn("existingVerificationId", pipeline)
        self.assertIn("if [ \"$existingVerificationId\" != \"$verificationId\" ]", pipeline)
        ...
        self.assertEqual(pipeline.count("azd deploy --no-prompt"), 2)
