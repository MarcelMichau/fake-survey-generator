...
class ConsolidatedFrontendTests(unittest.TestCase):
    ...
    def test_production_azd_configuration_deploys_only_the_api_service(self) -> None:
        ...
        self.assertNotIn("Wait for Custom Domain DNS Records", pipeline)
        self.assertNotIn("for attempt in $(seq 1 30)", pipeline)
        self.assertNotIn("dig +short CNAME", pipeline)
        self.assertNotIn("dig +short TXT", pipeline)
