...
class ConsolidatedFrontendTests(unittest.TestCase):
    ...
    def test_production_azd_configuration_deploys_only_the_api_service(self) -> None:
        ...
        self.assertIn("Wait for Custom Domain DNS Records", pipeline)
        self.assertIn("for attempt in $(seq 1 30)", pipeline)
        self.assertIn("dig +short CNAME", pipeline)
        self.assertIn("dig +short TXT", pipeline)
        self.assertIn("properties.customDomainVerificationId", pipeline)
        self.assertIn("asuid.fakesurveygenerator.mysecondarydomain.com", pipeline)
