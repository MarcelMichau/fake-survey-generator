...
public sealed class IsUserRegisteredQueryHandler(SurveyContext context)
    : IRequestHandler<IsUserRegisteredQuery, Result<UserRegistrationStatusModel>>
{
    private readonly SurveyContext _surveyContext = context ?? throw new ArgumentNullException(nameof(context));

    public async Task<Result<UserRegistrationStatusModel>> Handle(IsUserRegisteredQuery request,
